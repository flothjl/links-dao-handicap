...
contract LinksHandicap is Ownable{
    ...
    mapping (address => uint) ownerScoreCount;
    ...
    function addScore(int8 value, uint256 scoretime, string memory courseName) public onlyLinksOwner{
    ...
    function addScoreBatch(Score[] memory _scores) public {
        for (uint i=0; i<_scores.length; i++) {
            addScore(_scores[i].score, _scores[i].scoretime, _scores[i].courseName);
        }
    }

    function getScoresByOwner(address _address) public view returns (Score[] memory) {
        return scores[_address];
    }

    function getScoreCountByOwner(address _address) public view returns (uint) {
